...
Windows 使用 `gradlew.bat build`。Gradle JVM 或 `JAVA_HOME` 需设置为 Java 21。

## 验证

`build` 会运行独立回归测试，并检查发行 JAR 的版本、NeoForge 元数据、Mixin 配置及 JSON 资源。单独运行回归和服务端测试：
...
Windows 将 `./gradlew` 替换为 `gradlew.bat`。`src/test/java` 下的 `*Test.java` 通过 `public static void main(String[] args)` 运行，测试失败时抛出异常或返回非零退出码。`src/gameTest` 是独立测试模组，不进入发行 JAR。

客户端界面和相机流程可用 `runClientSmoke` 检查，运行前将测试世界放入 `run-client-smoke/saves/port-test`，不要使用正式存档。涉及联机的改动还需使用独立服务端测试。PR 中说明测试环境、结果和未测试的部分。
